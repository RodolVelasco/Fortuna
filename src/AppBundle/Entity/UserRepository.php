...
class UserRepository extends EntityRepository
{
    ...
    public function search($searchParam) {
        ...
        if(!empty($keyword))
            $qb->andWhere('concat(u.apellido, u.nombre) like :keyword or u.email like :keyword or u.username like :keyword or u.roles like :keyword')
                ->setParameter('keyword', '%'.$keyword.'%');

        if(!empty($ids))
            $qb->andWhere('u.id in (:ids)')->setParameter('ids', $ids);
        if(!empty($sortBy)){
            $sortBy = in_array($sortBy, array('nombre', 'apellido', 'username')) ? $sortBy : 'id';
